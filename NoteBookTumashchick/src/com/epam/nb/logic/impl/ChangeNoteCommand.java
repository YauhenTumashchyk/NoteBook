...
public class ChangeNoteCommand implements Command {
	...
	public Response execute(Request request) throws LogicException {
		...
		String changedNoteValue = (String) request.getParam(RequestParam.CHANGED_NOTE_REQUEST);
		...
		try {
			dao.changeNote(changedNoteValue, title, author);
		...
		response.setStatusMessage("Change Note Request confirmed");
		return response;
	}
